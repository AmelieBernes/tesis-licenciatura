...
dada por
\[
\Sigma_{x}(\omega) = \sigma_{n, \omega} (x) \hspace{0.2cm}
\text{ para toda }
\hspace{0.2cm} \omega \in \left[ 0, \frac{n}{2}\right],
\]
...
De forma teórica no hay ningún problema en 
definir el dominio de espectros
como los descritos en la definición 
\ref{def: espectro monofrecuenciales} 
como el intervalo $\left[ 0, \frac{n}{2}\right]$, pero, para
poder calcular en realidad 
espectros de una señal, se debe de usar
...
\begin{itemize}
	...
	(y denotaremos
	por $FP1(x)$) a una frecuencia $\omega \in [0, \frac{n}{2}]$ 
...
\begin{nota}
Note que la existencia de frecuencias principales para
ambos espectros (como se definieron en
\ref{def: FM0} y \ref{def: FM1})
están aseguradas por estár estas definidas
en términos de máximos de
conjuntos finitos de números, pero no estamos asegurando
que tales frecuencias sean únicas.
Si hay dos o más frecuencias $\omega'$ que satisfagan
la definición de frecuencia principal-1, tomamos
como valor de $FP0(x)$ a la mayor de tales $\omega'$.
Hacemos lo mismo para determinar el valor de 
$FP1(x)$. 
\end{nota}
