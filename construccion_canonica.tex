...
\begin{figure}[H]
	...
	A la derecha, la gráfica de $\phi(t)=0.5t-2$. En lo que sigue, a  
    funciones lineales como $\phi$, que se
    definen para obtener los puntos de una malla
    uniforme a partir de los de otra, les llamaremos
    \textbf{funciones de cambio
	de malla}.}
 \end{figure}
\QEDB
\vspace{0.2cm}
...
de dimensión $n$ con dominio uniforme y
de grado a lo más $i$ 
\sidenote{
Una contención se 
...
%	\centering
%	\includegraphics[scale=1]{25Nov_1} 
%	\caption{Lo que sí es seguro es que la unicidad establecida en la proposición \ref{prop: el operador de discretizacion puntual es un isomorfismo (...)} no es válida si se quita la restricción en los grados de los polinomios a discretizar.}
%	\end{figure}	


\begin{SCfigure}[\sidecaptionrelwidth][H]
	\parbox{\textwidth}{\centering\includegraphics[scale=0.5]{25Nov_1}}
	%\includegraphics[scale=0.5]{25Nov_1} 
	\sidecaption{Lo que sí es seguro es que la unicidad establecida en la proposición \ref{prop: el operador de discretizacion puntual es un isomorfismo (...)} no es válida si se omite la restricción en los grados de los polinomios a discretizar.}
\end{SCfigure}
