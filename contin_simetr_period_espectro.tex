...
\begin{figure}[H]
	\sidecaption{
	Una señal constante $x \in \IR^{n}$ es, por excelencia, aquella
	que no presenta oscilaciones. Por el contrario, la gráfica de
	$A_{n}(x)$ presenta el número máximo de oscilaciones posibles
	para una señal de tamaño $n$.
	\label{fig: alter 1}
	}
	\centering
	\includegraphics[scale = 0.45]{alter_1} 
\end{figure}	
...
\begin{figure}[H]
	...
	\centering
	\includegraphics[scale = 0.45]{alter_2} 
\end{figure}	

\begin{figure}[H]
	\sidecaption{
	Por el contrario, si la gráfica de $x \in \IR^{n}$ 
	presenta muchas oscilaciones, entonces la de6
	$A_{n}(x)$ es más suave. 
	\label{fig: alter 3}
	}
	\centering
	\includegraphics[scale = 0.45]{alter_3} 
\end{figure}	
...
%\input{limite} aquí están algunos resultados en limpio (sin desarrollo) de lo que calculé para poder determinar los límites extremos del espectro de una señal.
...
\marginnote{Claro que el codominio de $\Sigma_{x}$ es $[0,1]$,
pues esta función se define en función del coseno.}
\begin{defi}
...
La definición \ref{def: espectro monofrecuenciales inicial}
...
\subsection{Sobre la interpretación de los valores de $\Sigma_{x}$}
